...
subroutine set_vertical_bcs(LogRho,LogNS,Vel_GD,Temp, LogINS, iVel, VertVel)
  ...
  iAlt = -1
  Temp(iAlt) = Temp(iAlt+1)
  Vel_GD(iAlt, 1:3) = Vel_GD(iAlt+1, 1:3) 
  ...
  do iSpecies = 1, nSpecies
     ...
     if (.not. IsPhotoChemical(iSpecies)) then
        ...
        do iAlt = 0,-1,-1
           ...
           ! 3rd Order Mesh Coef
           MeshCoef0 = -1.0*( &
                MeshH2*MeshH3*MeshH4 + MeshH1*MeshH3*MeshH4 + &
                MeshH1*MeshH2*MeshH4 + MeshH1*MeshH2*MeshH3)/&
                (MeshH1*MeshH2*MeshH3*MeshH4) 
           MeshCoef1 =  1.0*( MeshH2*MeshH3*MeshH4)/&
                (h1*h2*(h2 + h3)*(h2 + h3 + h4))
           ...
           if (dLogNS .le. 0.0) LogNS(iAlt,iSpecies) = LogNS(iAlt+1,iSpecies)
  ...
  do iAlt = 0,-1,-1
     ...
     do iSpecies = 1, nSpecies
        ...
        dVertVel = &
             VertVel(iAlt+1,iSpecies)*MeshCoef0 + & 
        ...
        VertVel(iAlt, iSpecies) = &
             VertVel(iAlt+1,iSpecies) - dAlt_F(iAlt+1)*dVertVel 
  ...
  do iSpecies = 1, nSpecies
     if (IsPhotoChemical(iSpecies) .and. &
          (VertVel(1,iSpecies) > 0.0) .and. &
          (iSpecies /= iO_3P_) ) then 
  ...
  if (UseMsisBcs) then
     ...
     do iDir = iEast_, iNorth_
        dVertVel = &
             ...
             Vel_GD(iAlt+5,iDir)*MeshCoef4
        Vel_GD(iAlt  ,iDir) = &
             Vel_GD(iAlt+1,iDir) - dAlt_F(iAlt+1)*dVertVel 
     enddo
  ...
  do iSpecies = 1, nSpecies
     ...
     if (IsPhotoChemical(iSpecies) .and. (VertVel(nAlts,iSpecies) < 0.0) ) then
        VertVel(nAlts+1,iSpecies) = -1.0*VertVel(nAlts  ,iSpecies)
  ...
  do iAlt = nAlts + 1, nAlts + 2
     ...
     ! 3rd Order Mesh Coef
     MeshCoefm0 =  1.0*( &
          MeshHm2*MeshHm3*MeshHm4 + MeshHm1*MeshHm3*MeshHm4 + &
          MeshHm1*MeshHm2*MeshHm4 + MeshHm1*MeshHm2*MeshHm3)/&
          (MeshHm1*MeshHm2*MeshHm3*MeshHm4) 
     MeshCoefm1 = -1.0*( &
          MeshHm2*MeshHm3*MeshHm4)/(hm1*hm2*(hm2+hm3)*(hm2+hm3+hm4))
     ...
     MeshCoefm4 =  1.0*( &
          MeshHm1*MeshHm2*MeshHm3)/(MeshHm4*(hm2+hm3+hm4)*(hm3+hm4)*hm4)
  ...
  do iAlt = nAlts+1, nAlts+2
     LogRho(iAlt) = alog(sum(Mass(1:nSpecies)*exp(LogNS(iAlt,1:nSpecies))))
  enddo
