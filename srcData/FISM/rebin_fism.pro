...
c_a_to_r, idate,rt
jd = jday(fix(cyear),fix(cmon),fix(cday))
cdoy = chopr('00'+tostr(jd),3)

fismfile = file_search('/bigdisk1/Data6/Data/FISM/'+cyear+'/FISM*60sec*_'+cyear+cdoy+'_*.sav')
fismfile = fismfile(0)

if strpos(fismfile,'.sav') ge 0 then begin
    restore,fismfile
    newfile = '/bigdisk1/Data6/Data/FISM/BinnedFiles/'+cyear+'/fismflux'+cyear+cmon+cday+'.dat'
