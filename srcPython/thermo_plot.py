...
def plot_model_results():
    ...
    for itime, utime in enumerate(all_times):
        ...
        if args.winds:
            nStep = 4
            xwind = np.array(all_winds_x[itime].transpose())[::nStep, ::nStep]
            ywind = np.array(all_winds_y[itime].transpose())[::nStep, ::nStep]
            xp = np.array(x_pos)[::nStep]
            yp = np.array(y_pos)[::nStep]
            ax.quiver(xp, yp, xwind, ywind, scale = 10000.0)
        ...
        if (doPlotDataLoc):
            iTimeData = 0
            dtSave = np.abs((DataTime[0] - utime).total_seconds())
            for i, t in enumerate(DataTime):
        ...
        if args.cut == 'alt' and (plot_north or plot_south):
            ...
            if plot_north:
                ...
                if (doPlotDataLoc):
                    if (np.max(DataLat) > 45.0):
                        ax2.plot((DataLon + shift - 90.0) * np.pi/180.0,
                                90.0 - DataLat, color = 'yellow')
                        ax2.plot((DataLon[iTimeData] + shift - 90.0) * np.pi/180.0,
                                90 - DataLat[iTimeData],
                                'o', markersize = 10, color = 'yellow')

                # Add the winds, if desired
                if args.winds:
                    nStep = 4
                    ewind = np.array(all_winds_x[itime][:, mask_north].transpose())
                    nwind = np.array(all_winds_y[itime][:, mask_north].transpose())
                    xp = np.array(np.radians(x_pos + shift - 90.0))
                    yp = np.array(90.0 - y_pos[mask_north])

                    t, r = np.meshgrid(xp, yp)
                    xwind = -nwind * np.cos(t) - ewind * np.sin(t)
                    ywind = -nwind * np.sin(t) + ewind * np.cos(t)

                    nx, ny = np.shape(xwind)
                    for ix in range(0, nx, nStep):
                        nStepL = int(nStep / np.sin(yp[ix]*np.pi/180.0))
                        ax2.quiver(xp[::nStepL], yp[ix],
                                   xwind[ix][::nStepL],
            ...
            if plot_south:
                ...
                if (doPlotDataLoc):
                    if (np.max(DataLat) < -45.0):
                        ax3.plot((DataLon +shift - 90.0) * np.pi/180.0,
                                90.0 + DataLat, color = 'yellow')

                # Add the winds, if desired
                if args.winds:
                    nStep = 4
                    ewind = np.array(all_winds_x[itime][:, mask_south].transpose())
                    nwind = np.array(all_winds_y[itime][:, mask_south].transpose())
                    xp = np.array(np.radians(x_pos + shift - 90.0))
                    yp = np.array(90.0 + y_pos[mask_south])
